...
export function Sidebar() {
  ...
  return (
    <Collapsible.Content className="bg-gray-950 flex-shrink-0 border-r border-slate-600 h-screen relative group overflow-hidden data-[state=open]:animate-slideIn data-[state=closed]:animate-slideOut">
      <Collapsible.Trigger
        className={clsx(
          'absolute h-7 w-7 right-4 z-[99] text-slate-200 hover:scale-105 duration-200 inline-flex items-center justify-center',
          {
      ...
      >
        <nav className="flex mx-2 flex-col gap-8 text-slate-100">
          ...
          <section className="flex flex-col gap-px">
            <LinkContent to='/' children='Clientes'/>

            <LinkContent to='/create' children='Cadastrar cliente'/>

            <LinkContent to='/about' children='Sobre'/>
          </section>
        </nav>
      </div>
    </Collapsible.Content>
